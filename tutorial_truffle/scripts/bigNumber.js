...
function getBigNumberEx(exampleNumber) {
  ...
  console.log("exampleNumber with noExponents >>", exampleNumber.noExponents());

  const noExponentsExample = new BN(exampleNumber.noExponents());
  console.log("exampleNumber bignumber toString : ", noExponentsExample);

  const noExponentsExample2 = new BN(exampleNumber2.noExponents());
  console.log("exampleNumber2 bignumber toString : ", noExponentsExample);

  const addResult = noExponentsExample.add(noExponentsExample2).toString();
  console.log(
    "noExponentsExample.add(noExponentsExample2).toString() : ",
    addResult
  );

  const one_ether = web3.utils.fromWei("1", "ether");
  console.log("web3.utils.fromWei('1', 'ether') >>", one_ether);
  const one_ether_wei = web3.utils.toWei("1", "ether");
  console.log("web3.utils.toWei('1', 'ether') >>", one_ether_wei);
}

...
async function getBalance(contractAddress: string, account: string) {
  ...
  console.log(
    `Balance of ${account} is ${ethers.utils.parseUnits(
      balance.toString(),
      "ether"
    )}`
  );
  console.log(
    `Balance of ${account} is ${ethers.utils.formatEther(balance.toString())}`
  );
  console.log(
    `Balance of ${account} is ${ethers.utils.formatUnits(
      balance.toString(),
      18
    )}`
  );
